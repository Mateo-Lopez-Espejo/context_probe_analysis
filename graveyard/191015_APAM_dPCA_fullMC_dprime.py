...
"""
this was worthless, there is no value in fitting the dPCA for each monte carlo element. it wont lower the randome shuffle 
floor (as is the case with LDA) because it is not overfiting the noise of the late time bins (as with LDA)
"""
...
def dPCA_fullMC_fourway_analysis(site, probe, meta):
    recs = load(site)
    ...
    def fit_transformt(trialR):
        R = np.mean(trialR, 0)
        # center data
        centers = np.mean(R.reshape((trialR.shape[1], -1)), 1)[:, None, None]
        R -= centers
    ...
    dPCA_projection, dPCA_transformation = fit_transformt(trialR)
...
for site, probe in zip(['AMT029a', 'ley070a'],[5,2]):
    ...
    fourway = make_cache(function= dPCA_fullMC_fourway_analysis,
    ...
    twoway = make_cache(function= dPCA_fullMC_twoway_analysis,
                        func_args={'site': site, 'probe': probe, 'meta': meta},
    ...
    for analysis, a_name in zip([four, two], ['fourway', 'paired']):
        ...
        suptitle = f"{site} probe {probe} {a_name} dPCA_MC dprime {meta['raster_fs']}Hz"
        fig.suptitle(suptitle, fontsize=20)

        analysis = f"dPCA_MC_{a_name}_{meta['raster_fs']}Hz"
