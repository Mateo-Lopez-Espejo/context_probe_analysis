...
def single_cell_tstat_cluster_mass(site, contexts, probes, cluster_threshold, meta,
    ...
    for (nidx, cellid), (cpidx, (c0, c1)) in tqdm(
        ...
        shuf_trialR = np.empty((meta['montecarlo'], rep, 2, prb, tme))
        ...
        if False and cellid == 'CRD012b-13-1' and (ctx_pairs.index((3, 4)) == cpidx):
            alpha = 0.05
            ncomp = prb * len(ctx_pairs)
            alpha_corr = alpha / ncomp

            from src.visualization.fancy_plots import squarefy
            eg_idx = np.s_[goodcells.index(cellid), cpidx, 3 - 1, :]
            ...
            s = cpn_shuf_clstr_max[:, eg_idx[2], 0]
            p = pvalue[eg_idx]
            ...
            ci_c = quantiles['bf_cp'][eg_idx]
            ci = quantiles['none'][eg_idx]
