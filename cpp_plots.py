...
def signal_PSTH(signal, epoch_names='single', psth_kws=None):
    '''
    plots PSTHs for a CPP signal. Generates a figure with m  axes where m is the epochs ploted

    :param recording: a signal object with CPP formatted epochs (see cpp_epochs)
    ...
    if epoch_names == 'single':  # get epochs matching 'voc_x' where x is a positive integer
        reg_ex = r'\Avoc_\d'
        epoch_names = nep.epoch_names_matching(signal.epochs, (reg_ex))
    elif epoch_names == 'pair':  # get epochs matching 'Cx_Py' where x and y are positive integers
        reg_ex = r'\AC\d_P\d'
        epoch_names = nep.epoch_names_matching(signal.epochs, (reg_ex))
    elif isinstance(epoch_names, str):  # get epochs matching the specified regexp
        reg_ex = epoch_names
        epoch_names = nep.epoch_names_matching(signal.epochs, (reg_ex))
    elif isinstance(epoch_names, list):  # uses epoch_names as a list of epoch names.
        ep_intersection = set(epoch_names).intersection(set(signal.epochs.name.unique()))
        if len(ep_intersection) == 0:
            raise AttributeError("specified epochs are not contained in signal")
        pass
